...
(スライドが最初の状態 -- クリックする前)
1. 素朴なコード生成器において、このcc を２つのforループの上に移動するlet挿入を行
うよう、変形します。

(1回クリックして reset0-shift0 等を表示する)
2. そのため、このようにプログラムを変形します。
青字の shift0 と reset0 がコントロールオペレータです。
shift0 は reset0からshift0 までの評価文脈を切り取り `k` に束縛します．

(1回クリックして 評価文脈が緑色になり、k への束縛が表示される)
3. この緑になった部分が、shift0 によって切り取られる評価文脈で `k` に束縛されます．
k のあとの２重矢印 `<=` は継続変数k への特殊な代入で、最後のブラケット
で書かれたものは、評価文脈の穴、ホールを表します。

ここで、計算をワンステップすすめると、評価文脈が削除されます。

(1回クリックすると、評価文脈が削除される)
4. さらに計算をすすめて、
