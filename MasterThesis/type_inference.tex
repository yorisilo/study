...
answer type の列
である．また，
$\alpha^i$はレベル$i$の型変数，$\gamma$はclassifier変数，$\sigma_x$は$\sigma$変数である． % \oishi{$\sigma$変数とは何か説明する}
...
\begin{oframed}
  ...
  \item[2] $C$から$t^0=s^0$ か $t^1=s^1$の形の制約を選び，それを$A = B$とする．$C_1 = C - \{A = B\}$ とする．
    ...
    \item[2-2] $A \neq B$ で $A$ が型変数のとき，
      ...
      \item[2-2-2] 型 $B$ に $A$ が現れないなら，$\Theta_1 = [A := B]$とし，unify1($\Theta_1(C_1), \Theta_1(\Theta)$を呼び出す．
      \end{description}
